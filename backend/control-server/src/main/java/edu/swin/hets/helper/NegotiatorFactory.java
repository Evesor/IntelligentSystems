...
//        try {
//            parameters =  new ObjectMapper().readValue(input, InputParameters.class);
//        } catch (IOException e) {
//            throw new ExecutionException(new Throwable("Bad parameters for NegotiatorFactory"));
//        }
        switch (input.get(0)) {
    ...
    private INegotiationStrategy createLinearUtilityDecentNegotiator (IUtilityFunction utilityFun,
        ...
        if (params.size() != 3) {
            throw new ExecutionException(new Throwable("Bad input"));
        }
